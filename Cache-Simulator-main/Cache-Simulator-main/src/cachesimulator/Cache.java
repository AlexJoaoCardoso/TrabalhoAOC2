...
public class Cache {
    ...
    private void addCache(String indice, String tag) {
        int endereco = 0;
        if(this.nSets > 1){
            endereco = Integer.parseInt(indice, 2) % this.nSets;
        }
        ...
        for (int i = 0; i < this.assoc; i++) {
            ...
            if (i == this.assoc - 1) {// verifica o miss é de capacidade ou de conflito por meio de um função que verifica se a cache está vazia ou não
                if (verificaCacheCheia()) {
                    this.missCapacidade++; // cache cheia = miss de capacidade
                } else {
                    this.missConflito++; // cache cheia = miss de conflito
                }
                cache.get(endereco).set(random(), tag); //tira um número randomicamente e o substitui
            }
        }
    }
        ...
        for (int f = 0; f < this.nSets; f++) {
            for (int j = 0; j < this.assoc; j++) {
                if (cache.get(f).get(j) == null) {
                    return false; // cache não está cheia
                }
            }
        }
